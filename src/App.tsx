...
export default function App() {
  ...
  const navItems = useMemo<StaffNavItem[]>(() => {
    ...
    if (currentUser.role === "waiter") {
      return [
        { id: "waiter_kitchen", label: "Kitchen Panel", icon: ChefHat },
    ...
    if (currentUser.role === "chef") {
      return [
        { id: "admin_kitchen", label: "Kitchen Panel", icon: ChefHat },
    ...
    if (currentUser.role === "manager") {
      return [
        { id: "admin_dashboard", label: "Manager Panel", icon: LayoutGrid },
        { id: "admin_kitchen", label: "Kitchen Panel", icon: ChefHat },
    ...
    return [
      ...
      {
        id: "admin_staff",
        label: "Staff (Managers, Chef, Waiters)",
        icon: UsersIcon,
      },
      { id: "admin_tables", label: "Tables", icon: Grid3X3 },
      {
          ...
          {!currentUser ? (
            ...
            >
              <StaffLayout
                ...
                navItems={navItems}
              >
                {staffView === "super_dashboard" && <SuperDashboard />}
                {staffView === "super_businesses" && <SuperBusinesses />}
                ...
                {staffView === "waiter_kitchen" && (
                  <AdminKitchen currentUser={currentUser} />
                )}
                {staffView === "waiter_tables" && (
                  <WaiterTables currentUser={currentUser} />
                )}
                ...
                {staffView === "admin_reports" && (
                  <AdminReports currentUser={currentUser} />
                )}
                {staffView === "profile" && currentUser && (
                  <Profile user={currentUser} onUpdated={setCurrentUser} />
                )}
              </StaffLayout>
            </motion.div>
          )}
