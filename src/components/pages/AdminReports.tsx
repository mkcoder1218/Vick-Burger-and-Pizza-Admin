...
type SalesReport = {
  ...
  totalOrders: number;
  totalRevenue: string;
  categoryBreakdown: { categoryId: string; categoryName: string; totalQuantity: number; totalRevenue: string }[];
};
...
export default function AdminReports({ currentUser }: { currentUser: User }) {
  ...
  const { data, isLoading } = useGetAll<ApiResponse<SalesReport>>(businessId ? REPORT_RESOURCE : null, { period, businessId });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <select value={period} onChange={(e) => setPeriod(e.target.value)} className="px-4 py-2 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange">
          {periods.map((p) => (
      ...
      <div className="bg-white rounded-3xl border border-zinc-200 overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-zinc-50 text-xs font-bold text-zinc-500 uppercase">
            <tr><th className="px-6 py-4">Category</th><th className="px-6 py-4">Quantity</th><th className="px-6 py-4">Revenue</th></tr>
          </thead>
          <tbody className="divide-y divide-zinc-100">
            {report?.categoryBreakdown?.map((c) => (
              <tr key={c.categoryId}>
                ...
                <td className="px-6 py-4">ETB {Number(c.totalRevenue).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
