...
export default function AdminDashboard({ currentUser }: { currentUser: User }) {
  ...
  const { data: analyticsData, isLoading: analyticsLoading } = useGetAll<ApiResponse<AnalyticsReport>>(businessId ? ANALYTICS_RESOURCE : null, { period, businessId });
  const { data: salesData, isLoading: salesLoading } = useGetAll<ApiResponse<SalesReport>>(businessId ? SALES_RESOURCE : null, { period, businessId });
  const { data: ordersData, isLoading: ordersLoading } = useGetAll<OrdersResponse>(businessId ? ordersByBusiness(businessId) : null);
    ...
    <div className="space-y-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          { label: 'Avg Order Value', value: `ETB ${Number(avgOrder).toFixed(2)}`, icon: TrendingUp },
          { label: 'Revenue (Weekly)', value: `ETB ${Number(totalRevenue).toFixed(2)}`, icon: ChefHat },
        ].map((s, i) => (
          <div key={i} className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm">
            ...
            {(analyticsLoading || salesLoading) ? <Skeleton className="h-8 w-28" /> : <p className="text-3xl font-serif text-[#C5A059]">{s.value}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-zinc-100 overflow-hidden shadow-sm">
        <div className="p-8 border-b border-zinc-50 flex justify-between items-center">
          ...
          <div className="text-xs font-bold uppercase tracking-widest text-zinc-400">Live</div>
        </div>
        <div className="divide-y divide-zinc-50">
          ...
          {!ordersLoading && recentOrders.map((o) => (
            <div key={o.id} className="p-8 flex items-center justify-between hover:bg-zinc-50/50 transition-all group">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-400 font-medium">{o.orderItems?.length ?? 0} items - ETB {Number(o.totalAmount).toFixed(2)}</p>
                </div>
              </div>
