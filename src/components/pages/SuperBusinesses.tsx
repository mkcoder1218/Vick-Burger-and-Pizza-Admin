...
export default function SuperBusinesses() {
  ...
  const { data, error, isLoading } = useGetAll<ListResponse<ApiBusiness>>(RESOURCE, { q: q || undefined, limit: 50, offset: 0, sortBy: 'createdAt', sortOrder: 'desc' });
  const businesses = useMemo(() => data?.data ?? [], [data]);
  const paged = useMemo(() => businesses.slice(page * limit, (page + 1) * limit), [businesses, page]);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <Button onClick={() => setOpen(true)} className="flex items-center gap-2"><Plus size={18} /> Register Business</Button>
        </div>
      </div>
      ...
      {isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white p-6 rounded-3xl border border-zinc-200">
      ...
      {error && <div className="text-sm text-red-500">Failed to load businesses</div>}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {paged.map((b) => (
          <div key={b.id} className="bg-white p-6 rounded-3xl border border-zinc-200 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-zinc-100 rounded-2xl flex items-center justify-center overflow-hidden">
                  {b.logoFile?.url ? (
                    <img src={buildUrl(b.logoFile.url)} className="w-full h-full object-cover" alt="logo" />
                  ) : (
                ...
                <div>
                  ...
                  {b.logoFileId && <p className="text-[10px] text-zinc-400 mt-1">Logo File ID: {b.logoFileId}</p>}
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => openEdit(b)} className="text-zinc-400 hover:text-black"><Edit2 size={18} /></button>
                <button onClick={() => removeBusiness(b.id)} className="text-zinc-400 hover:text-red-500"><Trash2 size={18} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="secondary" onClick={() => setPage((p) => (p + 1))} disabled={(page + 1) * limit >= businesses.length}>Next</Button>
      </div>

      <Drawer open={open} title="Register Business" onClose={() => setOpen(false)}>
        <form className="space-y-5" onSubmit={submitCreate}>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Business Name</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Address</label>
            <input value={address} onChange={(e) => setAddress(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Phone</label>
            <input value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Logo</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Chapa Public Key</label>
            <input value={chapaPublicKey} onChange={(e) => setChapaPublicKey(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Chapa Secret Key</label>
            <input value={chapaSecretKey} onChange={(e) => setChapaSecretKey(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
      ...
      <Drawer open={editOpen} title="Edit Business" onClose={() => setEditOpen(false)}>
        <form className="space-y-5" onSubmit={submitEdit}>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Address</label>
            <input value={editAddress} onChange={(e) => setEditAddress(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Phone</label>
            <input value={editPhone} onChange={(e) => setEditPhone(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Logo</label>
            <label className="group block border-2 border-dashed border-burger-black/10 rounded-2xl p-4 cursor-pointer hover:border-burger-orange transition">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Chapa Public Key</label>
            <input value={editChapaPublicKey} onChange={(e) => setEditChapaPublicKey(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-burger-black/40">Chapa Secret Key</label>
            <input value={editChapaSecretKey} onChange={(e) => setEditChapaSecretKey(e.target.value)} className="w-full px-4 py-3 rounded-2xl border-2 border-burger-black/10 focus:outline-none focus:border-burger-orange" />
          </div>
